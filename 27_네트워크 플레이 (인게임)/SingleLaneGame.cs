...
public class SingleLaneGame : MonoBehaviour
{
    ...
    // 상대방 카드 설정 (네트워크)
    public void SetOpponentSelectedCard(string selectedCard)
    {
        you.SetSelectedCard(selectedCard);
    }
    ...
    public void ClickEndTurn()
    {
        ...
        {
            // 싱글 플레이, 멀티플레이
            if (GameManager.instance.gameMode == 0 || GameManager.instance.gameMode == 2)
            {
                if (me.CheckSelectedCard())
                {
                    ...
                    {
                        netplay.SendEndTurn(me.GetSelectedCard());
                    }
                }
                else
                {
